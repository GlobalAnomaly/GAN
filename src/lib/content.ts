 ...
 *
 * Every page reads through this module and nothing else touches the data
 * source directly. Today it serves the hand-entered seed; when Supabase is
 * live, only the bodies of these functions change and no page is touched.
 *
 * All functions are async even though the seed is synchronous, so that the
 * swap does not turn into a rewrite of every call site.
 */
...
function byDateDesc(a: { date_of_event: string | null }, b: { date_of_event: string | null }) {
...
export async function listCases(
  ...
  if (filters.country)
    rows = rows.filter((c) => c.country === filters.country);
  if (filters.tag)
...
/**
 * Related cases share at least one tag, most overlap first. This is what
 * turns a single case into a trail worth following.
 */
...
/** The credibility hook on the home page. */
